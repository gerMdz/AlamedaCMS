$(document).ready(function () {
    var referenceSection = new ReferenceSection($('.js-section-list'))
...
class ReferenceSection {
    constructor($element) {
        ...
        this.$element.on('click', '.js-reference-delete', (event) => {
            this.handleReferenceDelete(event, this.$element.data('principal'));
        });
    ...
    handleReferenceDelete(event, principal) {
        const $li = $(event.currentTarget).closest('.list-group-item');
        const id = $li.data('id');
        $li.addClass('disabled');

        $.ajax({
            url: '/admin/section/principal/' + id +'/'+ principal,
            method: 'DELETE'
        }).then(() => {
