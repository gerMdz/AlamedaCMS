...
$(function () {
    claves.forEach(clave => {
        const valor = localStorage.getItem(`apre06P${clave}`);
        const $input = $(`#p${clave}`);
...
function procesa(clave) {
    ...
    localStorage.setItem(`apre06P${clave}`, JSON.stringify(val));
}

function aceptado(clave) {
    const val = JSON.parse(localStorage.getItem(`apre06P${clave}`));
    const $label = $(`#lp${clave}`);
    const $container = $(`#sinp${clave}`);
