...
$(function () {
    claves.forEach(clave => {
        const valor = localStorage.getItem(`tmv07P${clave}`);
        const $input = $(`#p${clave}`);
...
function procesa(clave) {
    ...
    localStorage.setItem(`tmv07P${clave}`, JSON.stringify(val));
}

function aceptado(clave) {
    const val = JSON.parse(localStorage.getItem(`tmv07P${clave}`));
    const $label = $(`#lp${clave}`);
    const $container = $(`#sinp${clave}`);
