...
$(function () {
    claves.forEach(clave => {
        const valor = localStorage.getItem(`amigo2025P${clave}`);
        const $input = $(`#p${clave}`);
...
function procesa(clave) {
    ...
    localStorage.setItem(`amigo2025P${clave}`, JSON.stringify(val));
}

function aceptado(clave) {
    const val = JSON.parse(localStorage.getItem(`amigo2025P${clave}`));
    const $label = $(`#lp${clave}`);
    const $container = $(`#sinp${clave}`);
